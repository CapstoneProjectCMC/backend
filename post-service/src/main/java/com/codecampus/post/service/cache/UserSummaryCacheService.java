...
import com.codecampus.post.mapper.UserSummaryMapper;
  ...
  public UserSummary getOrLoad(String userId) {
    ...
    try {
      if (lock.tryLock(2, 10, TimeUnit.SECONDS)) {
        ...
        ApiResponse<ProfileResponseDto> api =
            profileClient.internalGetUserProfileByUserId(userId);
        ProfileResponseDto profile = api != null ? api.getResult() : null;
        UserSummary summary =
            userSummaryMapper.toUserSummaryFromProfileResponseDto(profile);
        if (summary != null) {
          put(userId, summary);
          return summary;
        }
    ...
    // 3) fallback nếu không lock được
    try {
      ApiResponse<ProfileResponseDto> api =
          profileClient.internalGetUserProfileByUserId(userId);
      return userSummaryMapper.toUserSummaryFromProfileResponseDto(
          api.getResult());
    } catch (Exception ex) {
      log.warn("[UserCache] Fallback call profile-service error {}: {}", userId,
          ex.getMessage());
      return null;
    }
  }
    ...
    try {
      if (lock.tryLock(2, 10, TimeUnit.SECONDS)) {
        ApiResponse<ProfileResponseDto> api =
            profileClient.internalGetUserProfileByUserId(userId);
        UserSummary summary =
            userSummaryMapper.toUserSummaryFromProfileResponseDto(
                api.getResult());
        if (summary != null) {
          put(userId, summary);
        }
