...
#    build_push_java "$svc"
#  done
  for svc in ai-service submission-service chat-service gateway-service identity-service notification-service org-service payment-service post-service profile-service quiz-service search-service; do
    echo "Building $svc..."
    build_push_java "$svc"
  done
#  build_push_file_service
