    ...
    public void submitQuiz(
        ...
        for (AnswerDto answerDto : submitQuizRequest.getAnswersList()) {
            ...
            QuizSubmissionAnswer quizSubmissionAnswer =
                    new QuizSubmissionAnswer(
                            new QuizSubmissionAnswerId(submission.getId(),
                                    question.getId()),
                            submission, question,
                            question.optionById(answerDto.getSelectedOptionId())
                                    .orElse(null),
        ...
        /* ---------- Đồng bộ sang submission-service ---------- */
        submissionStub.createQuizSubmission(
                CreateQuizSubmissionRequest.newBuilder()
                        .setSubmission(submissionMapper.toGrpc(submission))
                        .build()
        );
